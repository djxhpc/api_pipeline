...
def _check_phash_suspected(filepath, filename, output_dir, db):
    ...
    若 Hamming 距離 <= PHASH_SUSPECTED_THRESHOLD，
    回傳 (similar_to, distance, phash_str, similar_to_sha256)；
    否則回傳 (None, None, phash_str, None)。
    """
    ...
    for fn, info in db.items():
        ...
        if not existing_ph:
            continue
        dist = ph_obj - imagehash.hex_to_hash(existing_ph)
    ...
    if best_match:
        ...
        return best_match, int(best_dist), ph_str, similar_to_sha256   # ← int() 轉換
    return None, None, ph_str, None
...
def run_ruler_classify(filepath):
    """
    尺規次分類（單張版）：判斷是「管線」、「人手孔」或「一般」三類。
    - 模型現已訓練為三類輸出。
    - 若信心度低於 RULER_CLASSIFY_CONF_THRESH、輸出非合法類別，或發生錯誤，
      皆回退為 "一般"。
    """
    try:
...
def process_single_image(filepath, output_dir):
    ...
    if PHASH_SUSPECTED_ENABLED and not is_duplicate:
        similar_to, distance, ph_str, similar_to_sha256 = _check_phash_suspected(
        ...
        if ph_str:
            ...
            _save_hashes_db(output_dir, db)
        if similar_to:
            # 寫入 hashes_db.json：similar_to 之後緊接 similar_to_sha256
