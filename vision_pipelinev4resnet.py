...
paths = {
    ...
    "ruler_classify": "./models/尺規管圓孔bestv3.pt",  
}
...
# 真平行框信心通常 >=0.69，雜訊 <=0.08；0.3 可乾淨分開（依實測資料調整）
HORIZONTAL_CONF_THRESHOLD = 0.3
VERTICAL_CONF_THRESHOLD   = 0.12

RULER_CLASSIFY_CONF_THRESH = 0.98
RULER_CLASSIFY_VALID = {"管子", "人手孔", "一般"}
...
def classify_image(filepath):
    ...
    # 先讀成 array 再推論：YOLO 以路徑載圖不支援 .jpe 等副檔名，且統一做 EXIF 校正
    res = model(_read_image_bgr(filepath), device=DEVICE, verbose=False)[0]
...
def _classify_crossing(vertical_boxes, horizontal_boxes):
    # 注意：is_crossed / best_iou 不可靠，僅供參考。
    # 尺規為細長物，真正交叉成「+」時外接框只在中央小區重疊(IoU極小)，
    # 沒碰到時反而可能因假框疊在垂直尺規上使 IoU 飆高 —— 實測證實 bbox
    # 幾何無法判斷物理交叉。status(正常/請重新拍攝) 僅依「有無垂直框」，未用到交叉。
...
def run_ruler(filepath):
    ...
    return {
        ...
        "detections": {
            ...
            "is_crossed_reliable": False,
            "is_crossed_note": "bbox 重疊推估，不可靠僅供參考；判定未使用此值"
        }
    }
...
def run_coord(filepath):
    ...
    class_id = 9
    try:
        pred = coord_yolo(_read_image_bgr(filepath), device=DEVICE, verbose=False)[0]
