...
# ===== HSV 色塊判斷相關設定 =====
MIN_COLOR_BLOB_PIXELS = 150     # 最大色塊若小於此像素數就視為雜訊，不予理會
CONTACT_DILATE_ITER   = 2       # 接觸判斷時的膨脹次數 (容許些微縫隙也算接觸)
CONTACT_DILATE_KSIZE  = 5       # 膨脹 kernel 大小

# 顏色 (BGR) — 與視覺化一致
COLOR_VERTICAL_BGR   = (0, 255, 0)   # 綠
COLOR_HORIZONTAL_BGR = (0, 0, 255)   # 紅
...
def _hsv_color_mask(img_bgr, color):
    """用 HSV 抓出指定顏色的二值遮罩"""
...
def _largest_blob(mask, min_pixels):
    """
    取出最大連通色塊 (面積 < min_pixels 視為雜訊回傳 None)
    回傳: (blob_mask(uint8) 或 None, area)
    """
    ...
    if best_label == -1 or best_area < min_pixels:
        return None, int(best_area)

    blob = (labels == best_label).astype(np.uint8) * 255
    return blob, int(best_area)


def _blobs_contact(green_blob, red_blob):
    """
    判斷兩個最大色塊是否接觸。
    用膨脹後再取交集，可容忍些微縫隙。
    回傳: (is_contact, overlap_pixels)
    """
    if green_blob is None or red_blob is None:
        return False, 0

    kernel = np.ones((CONTACT_DILATE_KSIZE, CONTACT_DILATE_KSIZE), np.uint8)
    g = cv2.dilate(green_blob, kernel, iterations=CONTACT_DILATE_ITER)
    r = cv2.dilate(red_blob,   kernel, iterations=CONTACT_DILATE_ITER)

    overlap = cv2.countNonZero(cv2.bitwise_and(g, r))
...
def _classify_crossing_hsv(color_img, has_vertical, has_horizontal):
    """
    color_img: 已把 vertical 畫綠、horizontal 畫紅的乾淨色圖
    回傳: (status, is_crossed, info)
    """
    if not has_vertical:
        return "None Detected", False, {}
    if not has_horizontal:
        return "Normal (Crossed)", False, {}

    green_mask = _hsv_color_mask(color_img, "green")
    red_mask   = _hsv_color_mask(color_img, "red")

    green_blob, green_area = _largest_blob(green_mask, MIN_COLOR_BLOB_PIXELS)
    red_blob,   red_area   = _largest_blob(red_mask,   MIN_COLOR_BLOB_PIXELS)

    is_crossed, overlap_px = _blobs_contact(green_blob, red_blob)

    info = {
        "green_area": green_area,
        "red_area":   red_area,
        "overlap_px": overlap_px,
        "green_valid": green_blob is not None,
        "red_valid":   red_blob is not None,
    }
    return "Normal (Crossed)", is_crossed, info
...
def run_ruler(filepath):
    ...
    # 乾淨的色圖：綠=vertical, 紅=horizontal (給 HSV 用)
    ...
    # 視覺化用
    vis_masks_data, vis_cls_names, vis_confs = [], [], []
    ...
    if boxes is not None and masks is not None:
        for i, box in enumerate(boxes):
            ...
            # 把 mask resize 回原圖尺寸再二值化
            mask_resized = cv2.resize(mask_np, (w, h), interpolation=cv2.INTER_LINEAR)
            mask_bool = mask_resized > 0.5

            if cls_name == "vertical" and conf >= VERTICAL_CONF_THRESHOLD:
                color_img[mask_bool] = COLOR_VERTICAL_BGR     # 綠
                vertical_count += 1
                ...
                color_img[mask_bool] = COLOR_HORIZONTAL_BGR   # 紅
                horizontal_count += 1
                vis_masks_data.append(mask_tensor)
                vis_cls_names.append(cls_name)
                vis_confs.append(conf)
    ...
    # ---- HSV 色塊接觸判斷 ----
    status, is_crossed, info = _classify_crossing_hsv(
        color_img,
        has_vertical=vertical_count > 0,
        has_horizontal=horizontal_count > 0
    )
    ...
    return {
        ...
        "detections": {
            ...
            "green_area":       info.get("green_area", 0),
            "red_area":         info.get("red_area", 0),
            "overlap_px":       info.get("overlap_px", 0),
        }
    }
